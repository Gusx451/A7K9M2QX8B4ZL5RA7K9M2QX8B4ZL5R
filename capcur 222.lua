...
local Library = loadstring(game:HttpGet("https://raw.githubusercontent.com/matheuzxs/RedzlibV5Modify/refs/heads/main/MelhorQueFluent"))()

local Window = Library:MakeWindow({
    Title = "Katana Hub - Brookhaven 🏡RP",
    SubTitle = "by: GuzTer & Anômino666",
    LoadText = "Katana Hub",
...
do
    ...
    sound.SoundId = "rbxassetid://14881005435"
...
Window:AddMinimizeButton({
    ...
    Button = { Image = "rbxassetid://18519896256", BackgroundTransparency = 0 },
...
-- Agora também aplica as cores: nome = Verde, bio = Vermelho
...
do
    ...
    pcall(function()
        ...
        -- Decide o nome a aplicar dependendo do usuário
        ...
        if LocalPlayer and LocalPlayer.Name then
            ...
            if LocalPlayer.Name == "LegendLuffy02" then
                ...
                roleBio = "Creator Script"

            elseif LocalPlayer.Name == "BAICON_320" then

                roleBio = "Vice Creator"
        ...
        -- Primeiro tenta enviar strings (ex: "Green"/"Red") — muitos remotes aceitam isso
        ...
        -- ALTERADO: azul -> verde (para deixar o nome em verde em vez de azul)tryApplyColors(Color3.fromRGB(0, 170, 255), Color3.fromRGB(75, 0, 130))
tryApplyColors(Color3.fromRGB(0, 170, 255), Color3.fromRGB(75, 0, 130))
...
MainTab:AddButton({
    ...
    Name = "Bem-Vindo No Katana Hub V8.3!",
...
-- AntiSit: impede que o jogador sente mesmo por frações de segundo
...
local function enableAntiForHumanoid(hum)
    ...
    -- se antiSit estiver ativo, desativa o estado Seated e força Sit = false
        ...
        if char then
            ...
            if hum then
                ...
                if antiSit then
                    ...
                    -- desativa o estado sentado e assegura que não fique sentado nem por curtíssimo tempo
...
game:GetService("RunService").Stepped:Connect(function()
    ...
    if char then
        ...
        if humanoid then
            ...
            -- garante AntiSit ativo também durante execução contínua
...
local TrollTab = Window:MakeTab({
    Title = "Troll",
    Icon = "rbxassetid://77438127237870"
})
...
local function jogarComSofa(alvo)
    ...
    task.spawn(function()
        ...
        while jogandoFling
            and alvo
            and alvo.Character
            and alvo.Character:FindFirstChild("Humanoid") do
            ...
            myHRP.CFrame =
                CFrame.new(alvoHRP.Position + alvoHRP.Velocity / 1.5)
...
local bringForce = nil

local function bringWithSofa(alvo)
    if not alvo or alvo == LocalPlayer then return end
    if not alvo.Character or not LocalPlayer.Character then return end

    local myChar = LocalPlayer.Character
    local myHRP = myChar:FindFirstChild("HumanoidRootPart")
    local myHum = myChar:FindFirstChildOfClass("Humanoid")

    local targetHRP = alvo.Character:FindFirstChild("HumanoidRootPart")
    local targetHum = alvo.Character:FindFirstChildOfClass("Humanoid")

    if not myHRP or not myHum or not targetHRP or not targetHum then return end

    ReplicatedStorage.RE
        :FindFirstChild("1Clea1rTool1s")
        :FireServer("ClearAllTools")
    ...
    local couch = myChar:FindFirstChild("Couch")
        or LocalPlayer.Backpack:FindFirstChild("Couch")
    ...
    bringForce = Instance.new("BodyVelocity")
    bringForce.MaxForce = Vector3.new(math.huge, math.huge, math.huge)
    bringForce.Velocity = Vector3.zero
    bringForce.Parent = myHRP

    task.spawn(function()
        while bringingPlayer
            and alvo.Character
            and alvo.Character:FindFirstChild("HumanoidRootPart") do
...
-- ================= ABA SKYBOX COM AUTO-RETORNO NO RESET =================
local SkyTab = Window:MakeTab({
    Title = "Skybox (OP)",
    Icon = "rbxassetid://100218868088071"
})
...
local SavedPosition = nil -- posição salva
local ResetBySkybox = false -- CONTROLE: só true quando clicar no Stop
...
-- Função para aplicar a camisa
local function AplicarCamisa(id)
    local character = player.Character
    if not character then return end
    
    local remote = ReplicatedStorage:FindFirstChild("Remotes")
        and ReplicatedStorage.Remotes:FindFirstChild("ChangeCharacterAppearance")
    if remote then
        pcall(function()
            remote:InvokeServer({["Shirt"] = id})
        end)
    end
...
-- ================= TELEPORTE APÓS RESET (APENAS SE FOR PELO STOP) =================
player.CharacterAdded:Connect(function(character)
    if ResetBySkybox and SavedPosition then
        local root = character:WaitForChild("HumanoidRootPart", 5)
        if root then
            RunService.RenderStepped:Wait()
            root.CFrame = SavedPosition
            AvatarManager:MostrarNotificacao("Skybox Desativada.")
        end

        -- limpa a flag para não afetar resets normais
        ResetBySkybox = false
    end
end)
...
SkyTab:AddSection({"Custom Skybox (OP)"})
...
SkyTab:AddButton({
    ...
    Callback = function()
        ...
        if SkyboxActive then
            -- Se clicar de novo enquanto ativo, só limpa animações
            if CurrentTrack then CurrentTrack:Stop() end
            if RigidTrack then RigidTrack:Stop() end
            SkyboxActive = false
        else
            -- salva posição ANTES de ativar
            if rootPart then
                SavedPosition = rootPart.CFrame
                AvatarManager:MostrarNotificacao("Skybox Ativada.")
            end
    ...
    Callback = function()
        ...
        if character then
            ...
            if hum and root then
                ...
                -- marca que o reset foi pelo botão
                ResetBySkybox = true
...
print("Lâmina Hub V6.2 (Verde) - Script carregado com RolePlayName/RolePlayBio e aba RGB aplicados.")---------------------------------------------------------
...
if TextChatService.ChatVersion == Enum.ChatVersion.TextChatService then
    ...
    TextChatService.TextChannels.RBXGeneral:SendAsync(
        ...
        ":Spawn Tornado Boat\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\rTornado has appeared!"
...
Tab14:AddButton({
    ...
    Callback = function()
        ...
        local TextChatService = game:GetService("TextChatService")
        if TextChatService.ChatVersion == Enum.ChatVersion.TextChatService then
            TextChatService.TextChannels.RBXGeneral:SendAsync(":Untornado Boat")
        end

        -- código original (sem modificar)
...
local function criarBotao(nome, comando)
    AdminTab:AddButton({
        Name = nome,
        Callback = function()
            if not selectedAdminPlayer then return end
...
criarBotao("Kick", "kick")

-- 🔁 BRING (JÁ APLICADO)
criarBotao("Bring", "bring")

-- 🖼️ TELAS
criarBotao("Tela Tubers93", "telatubers93")
criarBotao("UnTela", "untela")
...
local function FlingLocal()
    local hrp = plr.Character and plr.Character:FindFirstChild("HumanoidRootPart")
    if hrp then hrp.Velocity = Vector3.new(0,2000,0) end
end

local function SitLocal()
    local hum = plr.Character and plr.Character:FindFirstChildOfClass("Humanoid")
    if hum then hum.Sit = true end
end

local function FrozenLocal()
    local hum = plr.Character and plr.Character:FindFirstChildOfClass("Humanoid")
    if hum then hum.PlatformStand = true end
end

local function UnFrozenLocal()
    local hum = plr.Character and plr.Character:FindFirstChildOfClass("Humanoid")
    if hum then hum.PlatformStand = false end
end

local function SpinLocal()
    SpinPlayers[plr.Name] = true
end

local function UnSpinLocal()
    SpinPlayers[plr.Name] = nil
end

local function CaixaLocal()
    if CaixaPlayers[plr.Name] then return end
...
local function UnCaixaLocal()
    if CaixaPlayers[plr.Name] then
        CaixaPlayers[plr.Name]:Destroy()
        CaixaPlayers[plr.Name] = nil
    end
end

-- 🔁 BRING LOCAL
local function BringLocal()
    local char = plr.Character
    if not char then return end
    ...
    local adminName = plr:GetAttribute("LastBringAdmin")
    ...
    if adminHRP then
        hrp.CFrame = adminHRP.CFrame * CFrame.new(0,0,-3)
    end
end
...
local function OnMessageReceived(message)
    ...
    if not cmd or not alvo then return end

    -- guarda quem mandou o comando
    if cmd == "bring" and message.TextSource then
        local sender = Players:GetPlayerByUserId(message.TextSource.UserId)
        if sender then
            plr:SetAttribute("LastBringAdmin", sender.Name)
        end
    end

    if string.lower(alvo) == string.lower(plr.Name) then
        ...
        elseif cmd == "uncaixa" then UnCaixaLocal()
        elseif cmd == "kick" then plr:Kick("Você foi kickado por um admin.")
        elseif cmd == "bring" then BringLocal()
        end
    end
end
